...
# Check if 'Close' column is present and valid
if data.empty or "Close" not in data.columns or data["Close"].isnull().all():
    st.error("Live gold data is unavailable or incomplete. Please check again shortly.")
else:
    ...
    try:
        ...
        macd_trend = "Buy" if latest["MACD"] > latest["Signal"] else "Sell" if pd.notna(latest["MACD"]) and pd.notna(latest["Signal"]) else "N/A"
...
st.markdown("<hr><small style='color:gray;'>Live gold chart with smart indicators | Built by ChatGPT</small>", unsafe_allow_html=True)
