...
if data is None or data.empty:
    st.error("Gold data is unavailable. Please try again later.")
elif "Close" not in data.columns:
    st.error("Missing 'Close' column in gold data.")
elif data["Close"].dropna().empty:
    st.error("Gold 'Close' prices are all NaN. Cannot compute indicators.")
else:
    # Clean & calculate
...
st.markdown("<hr><small style='color:gray;'>Built for mobile traders. Powered by real-time XAU/USD data.</small>", unsafe_allow_html=True)
