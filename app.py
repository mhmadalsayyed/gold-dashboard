...
if data is None or data.empty or "Close" not in data:
    st.error("Gold price data is not available. Please try again later.")
else:
    data.dropna(inplace=True)

    # Calculate indicators only if data is present
    if not data.empty and "Close" in data.columns:
        ...
        st.warning("Data is present but incomplete for calculation.")

st.markdown("<hr><small style='color:gray;'>Live gold signals powered by real-time data. Developed by ChatGPT</small>", unsafe_allow_html=True)
